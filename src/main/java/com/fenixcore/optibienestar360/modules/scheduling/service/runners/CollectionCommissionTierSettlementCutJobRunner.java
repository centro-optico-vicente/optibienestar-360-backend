...
import com.fenixcore.optibienestar360.modules.promoter.service.CommissionPeriodicSettlementService.CollectionSettlementOutcome;
...
/**
 * Daily automation of {@link CommissionPeriodicSettlementService#settleCollectionCut}
 * — phase 3 of the hub plan ("commission-frequency-currency-unification"),
 * closing the one remaining gap: {@link CollectionCommissionTier} (direct
 * collection commission, {@code AppliesTo.MONTHLY}) had no cut-close
 * primitive of its own — it was only ever paid via the generic {@code
 * CommissionPayoutService} sweep, ignoring its own configurable cut
 * frequency.
 *
 * <p>Walks every active {@link CollectionCommissionTier} and, for each one,
 ...
 * ({@code cut.end().equals(today)}) — {@code settleCollectionCut} mutates
 * {@code APPROVED} rows to {@code PAID}, so calling it before the cut has
 * actually closed would pay commissions early. Candidate promoters for that
 * cut are discovered via {@link CommissionRepository#findApprovedForPeriod}
 * (whole-window start through the cut's end), filtered to {@code
 * AppliesTo.MONTHLY}, grouped by promoter id, then {@code
 * settleCollectionCut} is invoked once per distinct promoter.</p>
 *
 ...
 * promoter on the same day and both call {@code settleCollectionCut} —
 * intentionally not deduplicated across rules, same as {@code
 * CommissionTierSettlementCutJobRunner}: a duplicate call over rows already
 * {@code PAID} finds nothing left {@code APPROVED} in the cut, so it's a
 * no-op, not a double payment.</p>
 */
@Component
@RequiredArgsConstructor
@Slf4j
public class CollectionCommissionTierSettlementCutJobRunner implements ScheduledJobRunner {

    public static final String CODE = "COLLECTION_COMMISSION_TIER_SETTLEMENT_CUT";
    ...
    public JobRunResult run() {
        ...
        List<CollectionCommissionTier> activeRules = collectionCommissionTierRepository.findByActiveTrue();
        ...
        for (CollectionCommissionTier rule : activeRules) {
            rulesProcessed++;
            ...
            for (Promoter promoter : distinctPromoters.values()) {
                CollectionSettlementOutcome outcome =
                        settlementService.settleCollectionCut(promoter, rule, today, payoutReference, false);
        ...
        log.info("COLLECTION_COMMISSION_TIER_SETTLEMENT_CUT completed: asOf={} rulesProcessed={} rulesAtCut={} "
