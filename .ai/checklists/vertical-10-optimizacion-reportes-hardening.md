...
- [ ] [P0/C3] **DB role `optisalud_public` (4to rol, post-modelado completo)** — least-privilege específico para el surface público. Cuando el modelado del schema esté congelado (V11..V27 aplicadas), agregar una nueva migración `V(N)__add_public_db_role.sql` que cree el rol y le otorgue acceso de SOLO LECTURA a la lista de tablas que aparecen en endpoints `/v1/public/*`:
    ...
    - `CREATE ROLE optisalud_public WITH LOGIN PASSWORD '${public_db_password}';`
    - `GRANT CONNECT ON DATABASE %I TO optisalud_public;`
    - `GRANT USAGE ON SCHEMA app TO optisalud_public;`
    - `GRANT SELECT ON <tabla>` por cada tabla habilitada (uno por uno — **NO** `GRANT SELECT ON ALL TABLES`).
    - `ALTER ROLE optisalud_public SET search_path = app, public;` (CVE-2018-1058).
    - `ALTER ROLE optisalud_public CONNECTION LIMIT ${public_conn_limit};` — más bajo que `app` ya que el tráfico público se cachea (recomendado: 10-20).
    - `ALTER ROLE optisalud_public SET statement_timeout = '${public_statement_timeout}';` — más estricto que app (recomendado: 5s; las queries públicas son simples y deben ser rápidas).
    ...
    - `@Primary` queda en el `optisalud_app` DataSource (admin/authenticated routes).
