...
- [ ] [P1/C2] `core/exception/LocalizedBusinessException.java` — clase abstracta con `String messageCode` + `Object[] args`. Heredan: `AuthenticationException`, `AccountLockedException`. Las que el JDK/Spring proveen (`IllegalArgumentException`, `NoSuchElementException`, `AccessDeniedException`) se siguen lanzando con el **code** como mensaje, y el handler lo detecta + resuelve.
- [ ] [P1/C3] `core/exception/GlobalExceptionHandler.java` — inyectar `MessageSource`; todos los `ProblemDetail` resolver `title` y `detail` por código contra `LocaleContextHolder.getLocale()`. Mapeo de status existente intacto (400/401/403/404/409/422/423/500).
- [ ] [P1/C2] `modules/auth/service/AuthService.java` — reemplazar 7 strings españoles por codes (`GENERIC_AUTH_ERROR`, `Token de refresco inválido o expirado`, `Token inválido o expirado`, `Usuario no encontrado`, `Contraseña actual incorrecta`, `La nueva contraseña no puede ser igual a las últimas X`).
- [ ] [P1/C1] `modules/auth/service/UserService.java` — 3 strings (Usuario no encontrado, Campo de filtro no permitido, El correo ya está registrado).
- [ ] [P1/C1] `modules/auth/service/RoleService.java` — 4 strings ingleses (`Role SYSTEM is not editable`, `Role not found:`, `Permission UUIDs not found:`, `Auto-lockout prevented:`).
- [ ] [P1/C2] Llenar `messages.properties` (canon EN), `messages_es.properties` (~25 claves), `messages_en.properties` (~25 claves) con todas las claves de `auth.*`, `user.*`, `role.*`, `error.title.*`, `error.detail.*`.
