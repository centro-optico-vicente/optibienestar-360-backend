...
- [x] [P0/C3] Validaciones custom: cédula VE (V-/E-), edad mín 18, tope beneficiarios = `plan.max_beneficiaries` _(v2: tope parametrizable, ver sección Adicionales v2)_ _(Infraestructura de Jakarta Validation completa para 2 de 3 reglas; la 3ra (tope) queda como check service-side cuando Plan exista. **`@VenezuelanDocumentNumber`** (`core/validation/`) — valida solo la parte numérica de la cédula con regex `^[0-9]{1,9}$` (1 a 9 dígitos, leading zero tolerado para data legacy; el prefix V/E va separado con su propio `@Pattern("^[VE]$")`). Tolera null/empty (presence via @NotBlank donde aplique). Aplicada a `AdminCreateUserRequest.documentNumber` + `AdminUpdateUserRequest.documentNumber`. Seed users "1" y "2" pasan. **`@MinimumAge(value=18)`** (default 18) — `Period.between(birthDate, now).getYears() >= minimumAge`; rechaza fechas futuras (negative period). Aún no aplicada (no hay DTO con birthDate); lista para futuro `MemberCreateRequest`. **Tope beneficiarios**: TODO service-side cuando Plan exista — el validator framework no encaja porque requiere DB lookup del plan del member. 2 keys i18n en 3 bundles. 10 tests en `CustomValidatorsTest` (template assertion locale-agnostic). 61/61 totales verdes.)_
